...
struct Server {
    ...
    Server() : acceptor(service), client(service) {
        acceptor.open(network::StreamProtocol::ip_v6());
        acceptor.bind(network::Endpoint(network::StreamProtocol::ip_v6(), 3005));
    }
...
void Server::Client::keep_reading() {
    socket.async_read_some(recv_buffer.get(), buffer_size, [this](std::error_code ec, size_t len) {
        if (ec == error::operation_aborted) {
            close();
            return;
        }
...
void Server::Client::on_message_complete() {
    ...
    ss << "HTTP/1.1 200 OK\r\n"
        ...
        "\r\n" << body;
    send_buffer = ss.str();
    socket.async_write(send_buffer.data(), send_buffer.size(), [this](std::error_code ec, size_t len) {
        responding = false;
        if (ec == error::operation_aborted) {
            close();
            return;
